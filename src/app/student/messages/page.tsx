...
export default function StudentMessagesPage() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setInput(messageText); // Restore input on fail
    } finally {
      setSending(false);
    }
  };
    ...
    <div className="h-[calc(100vh-12rem)] flex flex-col sm:flex-row gap-6 animate-in fade-in">
      ...
      <div className={`w-full sm:w-80 bg-white border rounded-3xl overflow-hidden shadow-sm flex flex-col shrink-0 ${activeContact ? 'hidden sm:flex' : 'flex'}`}>
         <div className="p-6 border-b bg-slate-50">
            <h2 className="text-xl font-bold font-heading flex items-center gap-2 mb-4">
            ...
            <>
               ...
               <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/30">
                  {messages.map((m: any, i: number) => {
                     // In student view, "me" is when sender.id == my userId (we can infer this if sender.role == STUDENT, but safer to check if it's NOT the participant)
                     // Since we don't have our own userId easily accessible without context, we know other participant is `activeContact.participant.id`

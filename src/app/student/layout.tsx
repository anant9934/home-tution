      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 border-t bg-white z-50 pb-safe">
         <div className="flex justify-around items-center h-16 px-2">
            {[
              ...
              { name: "Courses", href: "/courses", icon: Book },
              { name: "Quizzes", href: "/quizzes", icon: PenTool },
              { name: "Leader", href: "/leaderboard", icon: Trophy },
              { name: "Profile", href: "/profile", icon: User },
            ].map(item => (
              <Link key={item.name} href={item.href} className={`flex flex-col items-center justify-center w-16 gap-1 ${item.name === 'Home' ? 'text-primary' : 'text-muted-foreground'}`}>
                 <item.icon className="w-5 h-5" />

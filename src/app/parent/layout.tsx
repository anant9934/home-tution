  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 border-t bg-white z-50 pb-safe">
         <div className="flex overflow-x-auto items-center h-16 px-2 hide-scrollbar">
            {navItems.map(item => {
              const isActive = pathname === item.href || pathname.startsWith(`${item.href}/`);
              return (
                <Link key={item.name} href={item.href} className={`flex flex-col items-center justify-center min-w-[72px] flex-1 gap-1 ${isActive ? 'text-primary' : 'text-muted-foreground hover:text-foreground transition-colors'}`}>
                   <item.icon className="w-5 h-5" />

    ...
    <div className="space-y-8 pb-20 lg:pb-8 relative">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 relative z-10">
         {[
           ...
           { label: "Active Courses", value: data.stats.totalCourses, icon: BookOpen, color: "text-muted-foreground", bg: "bg-muted/50" },
         ].map((stat, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
         ...
         <div className="glass-card rounded-3xl col-span-1 p-6 relative overflow-hidden">
            <h3 className="font-heading font-bold text-lg mb-4">Pending Tutor Verifications</h3>
            <div className="divide-y divide-white/20 overflow-hidden">
               {data.pendingTutors.map((tutor: any) => (
                 <div key={tutor.id} className="py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all duration-300">
                    <div className="flex items-center gap-4">
                       <div className="w-12 h-12 bg-white/40 rounded-full shadow-sm flex items-center justify-center shrink-0">
                    ...
                    <div className="flex items-center gap-3">
                       <Badge variant="outline" className="bg-white/50">{tutor.subject}</Badge>
                       <div className="flex gap-1">
                          ...
                          <Button size="icon" onClick={() => handleRejectTutor(tutor.id, tutor.name)} variant="outline" className="w-8 h-8 rounded-full bg-white/50 hover:bg-destructive/10 hover:text-destructive hover:border-destructive/30 border-white/50">
         ...
         {/* RECENT TRANSACTIONS */}
         <div className="glass-card-premium rounded-3xl col-span-1 p-6 relative overflow-hidden">
            <h3 className="font-heading font-bold text-lg mb-4">Recent Transactions</h3>
            <div className="divide-y divide-white/20">
               {data.recentBookings.map((txn: any) => (
                 <div key={txn.id} className="py-4 flex items-center justify-between hover:bg-white/10 transition-colors rounded-xl px-2">
                    <div>
                    ...
                    <div className="text-right">
                       ...
                       <div className="text-xs font-semibold px-2 py-0.5 mt-1 rounded-md bg-success/10 text-success inline-block shadow-sm">
                         {txn.status}
                       </div>
                    </div>
                 </div>
            ...
            {data.recentBookings.length > 0 && (
               <div className="pt-4 mt-2 border-t border-white/20 text-center">
